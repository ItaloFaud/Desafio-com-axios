  ...
  render() {
    return (
      <div className="App">
        ...
        {this.props.valor === 0 ? "Sem valores ainda" : this.props.cotacoes.map((cotacao) => {
          return(
            <div><h6>O valor {this.props.valor} em {cotacao.name} ({cotacao.ask}), vale em Reais {(cotacao.ask*this.props.valor).toFixed(2)} </h6><br></br></div>
          )
        })
